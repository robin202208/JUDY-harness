...
function harnessWith(env: Record<string, string> = {}, extra: LaunchOverrides = {}): JudyHarness {
  const harness = new JudyHarness({ launch: fakeLaunch(env, extra) })
...
describe('JudyHarness', () => {
  ...
  it('supports await using disposal', async () => {
    let captured: JudyHarness
    {
      await using harness = new JudyHarness({ launch: fakeLaunch() })
      captured = harness
...
describe('HarnessClient', () => {
  ...
  it('delivers notifications to unfiltered and filtered subscriptions in wire order', async () => {
    ...
    expect(identity.serverInfo.name).toBe('judy-harness-sdk-runtime')
