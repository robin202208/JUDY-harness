"""Locate the bundled JUDY SDK runtime shipped with this package.
...
PACKAGE_METADATA_FILENAME = "judy-harness-runtime.json"
...
_EXE_ACQUISITION_HINT = (
    ...
    "judy-harness checkout, or install the matching `judy-harness-runtime-bin` platform "
...
def bundled_package_dir() -> Path:
    ...
    if not metadata.is_file():
        raise FileNotFoundError(f"judy-harness-runtime-bin is missing {metadata}")
    return root
...
def bundled_default_config_path() -> Path:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"judy-harness-runtime-bin is missing the default runtime config at {path}"
        )
    return path
...
def bundled_runtime_path() -> Path:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"judy-harness-runtime-bin is missing the runtime executable at {path}. "
            + _EXE_ACQUISITION_HINT
        )
    if tag.startswith("macos-"):
        ...
        if not helper.is_file():
            raise FileNotFoundError(
                f"judy-harness-runtime-bin is missing the node-pty spawn helper at {helper}. "
                + _EXE_ACQUISITION_HINT
            )
    return path
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    raise ValueError(
        f"unsupported JUDY runtime mode {selected!r}: expected 'exe' or 'node' "
...
def _node_launch_args() -> tuple[str, str]:
    ...
    if not bin_js.is_file():
        raise FileNotFoundError(
            ...
            f"(no {bin_js}); run `scripts/build-exe-for-python-sdk.ts` in a judy-harness "
