...
from judy_harness import JudyHarness, HarnessClient, HarnessConfig, Notification, SdkProtocolError
...
""".strip()
    ...
    with JudyHarness(launch_args_override=(sys.executable, str(script)), cwd=str(tmp_path)) as harness:
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from judy_harness import JudyHarnessConfig, Session
    ...
    assert "profile" not in inspect.signature(JudyHarness.run).parameters
    assert "profile" not in inspect.signature(Session.run).parameters
    assert "system_prompt" not in JudyHarnessConfig.__dataclass_fields__
    assert "max_tokens" in JudyHarnessConfig.__dataclass_fields__
...
""".strip()
    ...
    module_dir = tmp_path / "judy_harness_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "judy_harness_runtime", raising=False)
...
def test_client_reports_missing_bundled_runtime_dependency(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="Install judy-harness-runtime-bin"):
        HarnessClient().start()
